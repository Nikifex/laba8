...
double MainWindow::f(double x, double a, double b, double c)
{
    return a*sin(pow(x,2))+b*cos(pow(x,3))+c;
}
...
void MainWindow::drawGraph(bool notEmpty)
{
    ...
    for(double i = (double)leftX+step;i<=(double)rightX;i+=step) {
        if(!std::isnan(f(i,a,b,c))) {
            if(first[0]) {
                path.moveTo((i+Ox)*onePixelX,(Oy-f(i,a,b,c))*onePixelY);
                first[0] = false;
            }
            else
                path.lineTo((i+Ox)*onePixelX,(Oy-f(i,a,b,c))*onePixelY);
        }
    }
